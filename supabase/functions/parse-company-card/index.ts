...
Верни ТОЛЬКО валидный JSON-объект с полями:
{
  ...
  "detected_entity_type": "enterprise | ip | self_employed"
}

Правила определения detected_entity_type:
- "enterprise" — если это юридическое лицо (ООО, АО, ПАО, ЗАО, НКО, ГУП, МУП и т.д.), есть КПП, ОГРН из 13 цифр
- "ip" — если это индивидуальный предприниматель (в названии есть "ИП", "Индивидуальный предприниматель"), ИНН 12 цифр, ОГРНИП из 15 цифр, нет КПП
- "self_employed" — если это самозанятый (в документе упоминается "самозанятый", "НПД", "налог на профессиональный доход"), ИНН 12 цифр, нет КПП и ОГРН
